    ...
    <div className="bg-gray-50 h-screen">
      ...
      <div className="flex flex-row h-screen ">
        ...
        <div className="flex flex-row justify-center w-full bg-gray-50">
          <CitiesComponent setShowCreateCity = {setShowCreateCity} setCurrentCity={setCurrentCity}/>
        </div>
        {showCreateCity && <CreateCity setShowCreateCity = {setShowCreateCity} currentCity={currentCity}/>}
      </div>
                  ...
                  <tbody>
                    {cities.map((city, index) => (
                      <tr key={city.city_id}>
                        ...
                        <td className="px-4"><button className="hover:underline hover:underline-offset-1" onClick={() => onEdit(city)}>Edit</button></td>
                        <td className="px-4"><button className="hover:underline hover:underline-offset-1">Delete</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-2" onClick = {() => setShowCreateCity(true)}>
                  Add City
                </button>
              </div>
            </div>
